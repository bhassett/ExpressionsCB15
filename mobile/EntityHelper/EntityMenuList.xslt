...
<!-- ###################################################################################################### -->
	...
	<xsl:template match="Entity">
		...
		<item>
			<xsl:attribute name="Text"><xsl:value-of select="$pDisplayName"/></xsl:attribute>
			<xsl:attribute name="NavigateUrl">
        <xsl:value-of select="concat($prefix, '-', EntityID, '-', ise:FormatStringForLink(SEName))"/><xsl:text>.aspx</xsl:text>
      </xsl:attribute>
      ...
      <xsl:choose>
          ...
          <xsl:attribute name="NavigateUrl">
            <xsl:value-of select="concat($prefix, '-', EntityID, '-', ise:FormatStringForLink(SEName))"/><xsl:text>.aspx</xsl:text>
				...
				<xsl:otherwise>
          <xsl:attribute name="NavigateUrl">editentity.aspx?entityname=<xsl:value-of select="$entity"/>&amp;entityid=<xsl:value-of select="EntityID"/></xsl:attribute>
        </xsl:otherwise>
			</xsl:choose>
