...
(function ($) {
    ...
    var templates = {
        PAGING_NUM: "uploadPageButtonTemplateID",
        PAGING_PREV: "previousPageButtonTemplateID",
        PAGING_NEXT: "nextPageButtonTemplateID"
    };
    ...
    $.fn.CmsEditor = {
        ...
        scanImageEditable: function () {
            ...
            */
            ...
            $(config.contentTag).each(function () {
                ...
                if ($this.attr("type") == "button" || $this.attr("type") == "submit") { thisCMSEditorPlugin.handleInputButtonStringResource($this); }
        ...
        attachEventsListener: function () {
            ...
            $(this.selectorChecker(config.contentTag)).click(function () {
                thisCMSEditorPlugin.editImage($(this));
            });
        },
        ...
        showImageEditorDialog: function (key, value) {
            ...
            if (!$.browser.msie) { template = this.parseTemplateReturnHtml("uploadMainTemplateIDNonIE", textParam); }
            else { template = this.parseTemplateReturnHtml("uploadMainTemplateIDForIE", textParam); }
            ...
            this.resetAjaxFormUploadOption();
            this.initTabs();
            ...
            var dialogWidth = 650;
            if ($.browser.msie && $.browser.version == '7.0') { dialogWidth = 660; }
            ...
            var option = {
                ...
                open: function () {
                    ...
                    else { thisCMSEditorPlugin.initializeImageDataLoading(key, counter, entitytype); }},
                beforeClose: function (event, ui) { $(this).dialog("destroy").empty(); },
                close: function () { thisCMSEditorPlugin.imageUploadCleanup(); } };

                this.initUploadButtonsEvents();
                this.attachEventsListener();
                
                $(dialogId).dialog(option).dialog("open");
        },
        
        // paging
        initImagePaging: function (recentIndex) {
            var pagingData = this.getPagingNums();
            if (pagingData == null || pagingData.length == 0) return;

            var pagingContainer = $("#upload-page-ul");
            pagingContainer.empty();

            var html = "";
            // generate previous button
            html = this.parseTemplate(templates.PAGING_PREV, null);
            $(pagingContainer).append(html);

            // generate pagenum buttons
            html = this.parseTemplate(templates.PAGING_NUM, pagingData);
            $(pagingContainer).append(html);

            // generate next button
            html = this.parseTemplate(templates.PAGING_NEXT, null);
            $(pagingContainer).append(html);

            this.tryNavigationPageButtonHideShow();

            //attach events for each pager button
            this.attachEventsOnPagers(pagingData);

            var index = pagingData[0].Index;
            ...
            this.setPagingHighlight();

        },
        getPagingNums: function () {
        ...
        setPagingHighlight: function () {
            var uploadOptions = this.getAjaxFormUploadOption();
            var pageNum = parseInt(uploadOptions.imgIndex) + 1;
            $("#upload-page-ul").find("#btnPage_" + pageNum).removeClass("btn-primary");
        },
        ...
        initTabs: function (tab) {
            ...
            var config = getConfig();
            var tabSelected = tabEnum.large;
            if (tab != null) { tabSelected = tab; }

            var uploadOption = this.getAjaxFormUploadOption();
            uploadOption.size = this.getSizeName(tabSelected);
            uploadOption.sizeValue = tabSelected;

            var tabContainer = $(this.selectorChecker(config.imageEditorTabId));
            $(tabContainer).tabs({
                selected: tabSelected,
                select: function (event, ui) {
                    var uploadOption = thisCMSEditorPlugin.getAjaxFormUploadOption();
                    uploadOption.size = thisCMSEditorPlugin.getSizeName(ui.index);
                    uploadOption.sizeValue = ui.index;

                    thisCMSEditorPlugin.setActiveTab(ui.index + 1);
        ...
        resetAjaxFormUploadOption: function () {
        ...
        reloadImageData: function () {
            ...
            this.initTabs(this.getEnumSizeByDescription(uploadOption.size));
        ...
        initUploadButtonsEvents: function () {
            ...
            $('#ctrlFileUpload').change(function () {
                ...
                if (uploadOption.size == "medium" && uploadOption.entityType == 'product' && uploadOption.uploadType == uploadType.upload) {
            ...
            $("#btnImageUploadCancel").on("click", function () {
        ...
        updateImageDefaultSizeToDB: function (callBack) {
            ...
            AjaxCallWithSecurity("../ActionService.asmx/ImageUploadSetAsImageDefault", //url
                                "", //type
                                "", //methodname
                                "", //datatype
                                "", //contentype
                               param,
                               function (result) {
                                   if (result.d) { if (typeof callBack != 'undefined') { callBack(); } }
                                   else { alert('failed update image default size.'); }
                               },
        ...
        buildAjaxFormOption: function () {
            ...
            var uploadOptions = this.getAjaxFormUploadOption();
            if (uploadOptions.imageSource.indexOf("../") == 0) {
                uploadOptions.imageSource = uploadOptions.imageSource.replace("../", "");
            }
