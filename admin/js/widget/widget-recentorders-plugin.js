(function ($) {
    ...
    $.fn.RecentOrdersWidget = {
        ...
        displayContent: function () {
            ...
            object.displayLimit = 10; //display limit for orders

            var jsonData = JSON.stringify(object);
            $.ajax({
                type: "POST",
                url: '../ActionService.asmx/GetWebRecentOrders',
                data: jsonData,
                dataType: "json",
                ...
                success: function (result) {
                    ...
                    if (jsonResult != '') {
                        var orders = $.parseJSON(jsonResult);

                        window.setTimeout(function () {
                            $(content).empty();
                            
                            if (orders.length > 0) {
                                $(content).append(parseTemplate("recentorders-body", {
                                    ID: table,
                                    SalesOrderCodeHeader: "SalesOrder",
                                    CustomerHeader: "Customer",
                                    DateHeader: "Date",
                                    CurrencyHeader: "Currency",
                                    TotalHeader: "Total"
                                }));
                                $(selectorChecker(table)).append(parseTemplate("recentorders-content", orders));
