(function ($) {
    ...
    $.fn.StockAlertWidget = {
        ...
        displayContent: function () {
            ...
            object.displayLimit = 10; //display limit for products low in free-stock

            if (config.Threshold != '' && config.Threshold != null) {
                object.threshold = config.Threshold;
            }
            else {
                object.threshold = 10;
            }

            object.isActiveOnly = isActiveOnly;

            var jsonData = JSON.stringify(object);
            $.ajax({
                type: "POST",
                url: '../ActionService.asmx/GetProductsLowInFreeStock',
                data: jsonData,
                dataType: "json",
                ...
                success: function (result) {
                    ...
                    if (jsonResult != '') {
                        ...
                        window.setTimeout(function () {
                            ...
                            if (items.length > 0) {
                                $(content).append(parseTemplate("stockalert-body", { ID: table, ItemHeader: "Item", StockHeader: "Free-Stock" }));
                                $(selectorChecker(table)).append(parseTemplate("stockalert-content", items));
        ...
        attachEvents: function () {
            ...
            //settings widget
            $(selectorChecker(config.WidgetID) + " a[widget-control='activeOnly']").click(function () {
                isActiveOnly = true;
                $(this).attr("class", "selected");
                $(selectorChecker(config.WidgetID) + " a[widget-control='showAll']").removeClass('selected');
                thisObject.refresh();
            });

            $(selectorChecker(config.WidgetID) + " a[widget-control='showAll']").click(function () {
                isActiveOnly = false;
                $(this).attr("class", "selected");
                $(selectorChecker(config.WidgetID) + " a[widget-control='activeOnly']").removeClass('selected');
                thisObject.refresh();
            });
