...
ise.Controls.AddressControl.prototype = {
    ...
    clearStates: function () {
        var elemState = $getElement(this.id + "_WithStateState");
        if (elemState) {
            elemState.options.length = 0;
        }
    },

    switchCountry: function (callBackSetState) {
        ...
        if (null != elemCountry) {
            ...
            if (country.withState) {
                ...
                ise.Utils.showRow(this.id + '_WithStatePostalRow');
                ise.Utils.hideRow(this.id + '_WithoutStateCityRow');
                ise.Utils.hideRow(this.id + '_WithoutStateCountyPostalRow');

                var populateState = function (states) {

                    var thisControl = "#" + thisId + "_WithStateState";
                    ...
                    if (states.length == 0) {
                        ...
                        elemState.options[0] = new Option(ise.StringResource.getString('address.cs.21'), '0');
                        for (var ctr = 1; ctr <= states.length; ctr++) {
                            var state = states[ctr - 1];
                    ...
                    $(thisControl).removeAttr("style");

                    if (callBackSetState.type != 'change' && callBackSetState) { callBackSetState(); }
                }
...
//                }
                ...
                //--> the following line of segment was added to hide or display billing and shipping address states (dropdown)

                var billingStates = $("#ctrlBillingAddress_WithStateState").find("option").length;

                if (billingStates > 0) {

                    $("#ctrlBillingAddress_WithStateState").css("display", "");
                }

                var shippingStates = $("#ctrlShippingAddress_WithStateState").find("option").length;

                if (shippingStates > 0) {

                    $("#ctrlShippingAddress_WithStateState").css("display", "");
                }
                ...
                if (addressStates > 0) {
                    ...
                    $("#ctrlAddress_WithStateState").css("display", "");

                }


                //<-- billing and shipping address states (dropdown)
                ...
                ise.Utils.hideRow(this.id + '_WithStatePostalRow');
                ise.Utils.showRow(this.id + '_WithoutStateCityRow');
                ...
                if (addressStates == 0) {
                    ...
                    $("#ctrlAddress_WithStateState").css("display", "none");
            ...
            if (this.getIsPostalCodeOptional()) {
                this.lblWithStateCityStatePostalCaption.innerHTML = ise.StringResource.getString('createaccount.aspx.29');
                this.lblWithoutStatePostalCaption.innerHTML = ise.StringResource.getString('createaccount.aspx.30');
            }
            else {
                this.lblWithStateCityStatePostalCaption.innerHTML = ise.StringResource.getString('createaccount.aspx.31');
    ...
    getWithStateState: function () {
        var elem = $getElement(this.id + '_WithStateState');
        if (elem) {
            return elem.options[elem.selectedIndex].value;
        }
    ...
    setCountry: function (value, callbackSetState) {
        var elem = $getElement(this.id + '_Country');
        if (elem) {
            ...
            if (this.getCountry() != value) {
                ...
                this.switchCountry(callbackSetState);

            }
            else {
                if (callbackSetState) callbackSetState();
            }
    ...
    setValue: function (address) {
        ...
        var setStateCallBack = function () {
            ...
            if (callBackRefreshShippingMethod) callBackRefreshShippingMethod();

        };

        this.setCountry(address.country, setStateCallBack);
    ...
    setDisabled: function (blnValue) {
        ...
        if (blnValue == true || blnValue == false) {
            ...
            ise.Utils.DisableUnselectedDropdownItems(blnValue, $getElement(this.id + '_WithStateState'));
    ...
    saveAddress: function () {
        ...
        if (this.addressControl) {
            ...
            if (this.addressControl.validate(true)) {
                ...
                var info = this.addressControl.getValue();
                var idOfControlBeingHandled = "";

                if (info.state != "" || info.state != null || typeof (info.state) != undefined) {

                    idOfControlBeingHandled = this.addressControl.id + "_WithStatePostalCode";


                } else {

                    idOfControlBeingHandled = this.addressControl.id + "_WithoutStatePostalCode";

                }

                recheckPostalAfterSubmit(idOfControlBeingHandled, this, "");
...
ise.Validators.DisallowShippingToPOBoxesValidator.registerClass('ise.Validators.DisallowShippingToPOBoxesValidator', ise.Validators.BaseValidator);
...
ise.Validators.CountryValidator = function (controlId, errorMessage, invalidIndex, next) {
    ise.Validators.CountryValidator.initializeBase(this);
    this.control = $getElement(controlId);
    this.errorMessage = errorMessage;
    this.next = next;
...
ise.Validators.CountryValidator.registerClass('ise.Validators.CountryValidator');
ise.Validators.CountryValidator.prototype = {
    shouldEvaluate: function () {
        return true;
    },
