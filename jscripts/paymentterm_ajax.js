...
ise.Validators.CardNumberValidator.prototype = {
    validate : function() {
        ...
        if(this.cardNumberControl && this.cardTypeControl) {
            ...
            if(!this.isValid) {
                if(ccErrorNo == 0) { this.isValid = true; }
            }
            this.errorMessage = this.errorMessages[ccErrorNo];
        }
    },
    
    toString : function() { return 'Card Number Validator:' + this.cardNumberControl.id; }
}
...
ise.Controls.PaymentTermOption.prototype = {
    ...
    getPaymentTerm: function () { return this.term; },

    getPaymentMethod: function () { return this.paymentMethod; }
}
...
ise.Controls.PaymentTermControl.prototype = {
    setRequireTermsAndConditions: function (requireTermsAndConditions) { this.requireTermsAndConditions = requireTermsAndConditions; },

    getNoPaymentRequired: function () { return this.noPaymentRequired; },

    setNoPaymentRequired: function (noPaymentRequired) { this.noPaymentRequired = noPaymentRequired; },

    getTermsAndConditionsChecked: function () {
        var elemTermsAndConditionsChecked = $getElement(this.id + '_termsAndConditionsChecked');
        if (elemTermsAndConditionsChecked) { return elemTermsAndConditionsChecked.checked; }
        return true;
    },

    getPaymentTerm: function () {
        var elemTerm = $getElement(this.id + '_paymentTerm');
        if (elemTerm) { return elemTerm.value; }
        return ise.Constants.EMPTY_STRING;
    },

    setPaymentTerm: function (term) {
        var elemTerm = $getElement(this.id + '_paymentTerm');
        if (elemTerm) { elemTerm.value = term; }
    },

    getPaymentMethod: function () {
        var elemPM = $getElement(this.id + '_paymentMethod');
        if (elemPM) { return elemPM.value; }
        return ise.Constants.EMPTY_STRING;
    },

    setPaymentMethod: function (term) {
        var elemPM = $getElement(this.id + '_paymentMethod');
        if (elemPM) { elemPM.value = term; }
    },
    ...
    getCardType: function () {
        ...
        if (elem) { return elem.options[elem.selectedIndex].value; }
        return ise.Constants.EMPTY_STRING;
    },
    ...
    switchView: function () {
        ...
        if (current) {
            ...
            if (this.validationController.validationSummary) { this.validationController.validationSummary.HideSummary(); }
            ...
            if (current.getPaymentTerm().toUpperCase() == 'PURCHASE ORDER') {
               ...
               if(this.IsInPaymentOnline){
                   $("#lblRedirectCaption").html(ise.StringResource.getString('payment.aspx.53'));
               } else {
                   $("#lblRedirectCaption").html(ise.StringResource.getString('checkoutpayment.aspx.25'));
               }
               this.showRedirectPanel();
                ...
                this.hideCreditCardPanel();
                if (this.IsInOnePageCheckout) {
                    $("#lblRedirectCaption").html(ise.StringResource.getString('checkout1.aspx.109'));
                }
                else
                {
                    $("#lblRedirectCaption").html(ise.StringResource.getString('checkoutpayment.aspx.65'));
                }
                this.showRedirectPanel();
            }
            else {
    ...
    validate: function (clear) {
        if (this.requireTermsAndConditions && !this.getTermsAndConditionsChecked()) {
        ...
        if (this.validationController.validators.length > 0 && this.IsTokenization) { this.DoRemovalOfCCVValidation(); }
